...
/// (config POST + GCD lookup) and hands the same string to the WebView, so the
/// partner backend sees one consistent identity. Never leaks Dart/Flutter/
/// CFNetwork/Darwin tokens.
class StrideClient extends http.BaseClient {
  ...
  String _mobileSafari(String iosVersion) {
    ...
    return 'Mozilla/5.0 (iPhone; CPU iPhone OS $cpu like Mac OS X) '
        'AppleWebKit/${CoopConfig.webKitVersion} (KHTML, like Gecko) '
        'Version/${CoopConfig.safariVersion} Mobile/15E148 '
        'Safari/${CoopConfig.safariTail}';
  }
