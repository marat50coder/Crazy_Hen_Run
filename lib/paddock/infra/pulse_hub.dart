...
Future<void> chrBackgroundMessage(RemoteMessage _) async {}

/// Firebase Messaging (APNs) owner: boots FCM, waits for the APNs token,
/// extracts push destination URLs, and manages the notification permission
/// request. Cold-start taps are handled by SceneDelegate + ColdTapReader; this
/// hub covers foreground / background-tap and token refresh.
class PulseHub {
  ...
  Future<void> _boot() async {
    ...
    FirebaseMessaging.onBackgroundMessage(chrBackgroundMessage);
  ...
  Future<bool> askPermission() {
    return _permissionFuture ??= _performPermissionRequest()
  ...
  Future<bool> _performPermissionRequest() async {
    ...
    if (accepted) {
      // The APNs token takes noticeably longer to arrive right after consent.
